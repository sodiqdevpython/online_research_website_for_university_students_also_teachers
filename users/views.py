...
def signup_redirect(request):
    messages.error(request, "Nimadir xato ketdi balki sizda allaqachon ushbu nom bilan hisobingiz bordir ?")
...
def custom_login(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                messages.success(request, f"Salom <b>{user.username}</b> hisobingizga muvaffaqiyatli kirdingiz !")
                return redirect("homepage")

        else:
            for key, error in list(form.errors.items()):
                if key == 'captcha' and error[0] == 'This field is required.':
                    messages.error(request, "Siz reCAPTCHA ni belgilashingiz kerak")
                    continue
...
def profile(request, username):
    if request.method == "POST":
        ...
        if form.is_valid():
            user_form = form.save()
            messages.success(request, f"{user_form.username}, Ma'lumotlaringiz yangilandi")
